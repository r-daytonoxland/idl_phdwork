...
;
; ** Currently this routine only models the (8-3) band,
;    and only P and Q branches **
;
...
common _synth_oh, OH_file,OH_range,lab,A,E,J,w
...
if (n_elements(OH_file) lt 1) || (OH_file ne file) || ~array_equal(OH_range, range) then begin
 ...
 while ~eof(lun) do begin
  ...
  ; Don't include transitions with different F-levels, or for very large J
  if (dat[4] ne dat[5]) || (float(dat[2]) gt 9) then continue
  ; For Q-branch, don't include L>3
  if (dat[2] eq dat[3]) && ((float(dat[2])+float(dat[4])-1.5) gt 3) then continue
  w[i]=vac2air(1d8/double(dat[9]))
  A[i]=double(dat[12])
  E[i]=(double(dat[11])+double(dat[9]))*100 ; Need upper state energy, so add wavenumber to lower state energy, then convert to m^-1
  J[i]=double(dat[2])
  lab[i]=oh_label(double(dat[0:5]))+dat[7]
  i+=1
 endwhile
 close,lun
